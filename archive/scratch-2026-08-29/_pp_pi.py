...
async def main():
    ...
    async with AsyncSession(impersonate='chrome131', verify=False) as s:
        ...
        print('PI id:', pi.get('id'))
        print('PI status:', pi.get('status'))
        print('PI amount:', pi.get('amount'), pi.get('currency'))
        print('PI client_secret:', (pi.get('client_secret') or 'НЕТ')[:50])
        print('session status:', data.get('status'), '| payment_status:', data.get('payment_status'))
        print('mode:', data.get('mode'), '| total:', data.get('total_summary', {}).get('total', {}).get('amount_subtotal', '?'))
